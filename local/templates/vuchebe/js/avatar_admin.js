$(document).ready(function() {
    ...
    $('.js-avatar-list').on('click', function () {

        if ($(this).hasClass('color-silver')) {
            return false;
        }

        $(".js-avatar-list").removeClass('color-silver');
    ...
    function lazyListAvatarAdmin() {
        ...
        $.ajax({
            url: '/ajax/lazy_load_avatar_admin.php',
            method: 'POST',
            ...
            if (data.res && data.res.length > 0) {
                ...
                html += `
                  ...
                  <div class="col-9 width-sm content-right">
                    <div class="news-name">
                        <span><a href="/user/${this['URL']}/" class="display-name" target="blank">${this['FORMAT_NAME']}</a></span>
                    </div>
                    <div style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">
                        ...
                        <div class="params-banner">Предыдущие жалобы: <a href="#" class="js-avatar-action abuse" data-id="${this['ID']}" data-user="${this['AUTHOR']}" data-type="abuse">${this['TOTAL']}</a></div>
                        <div class="params-banner">Предупреждений: <a href="#" class="js-avatar-action warning" data-id="${this['ID']}" data-user="${this['AUTHOR']}" data-type="warning">${this['WARNING_CNT']}</a></div>
                        <div class="params-banner">Отклонённые жалобы: <a href="#" class="js-avatar-action reject" data-id="${this['ID']}" data-user="${this['AUTHOR']}" data-type="reject">${this['REJECT_CNT']}</a></div>
                        <div class="params-banner">Удаление аватара: <a href="#" class="js-avatar-action del" data-id="${this['ID']}" data-user="${this['AUTHOR']}" data-type="del">${this['DEL_CNT']}</a></div>
                        <div class="params-banner">Созданных тикетов: <a href="#" class="js-avatar-action ticket" data-id="${this['ID']}" data-user="${this['AUTHOR']}" data-type="ticket">${this['TICKET_CNT']}</a></div>
                    </div>
                  </div>
                  <div class="params-banner-top col-12" style="margin-top: 15px; text-align: right;">
                        ${this['DEL'] !== 'Y' ? `<a class="color-silver js-avatar-button del-avatar" data-type="del-avatar" data-id="${this['ID']}" data-user="${this['AUTHOR']}">Удалить аватар</a>` : ``}
                        ${this['REJECT'] !== 'Y' ? `<a class="color-silver js-avatar-button reject" data-type="reject" data-id="${this['ID']}" data-user="${this['AUTHOR']}">Отклонить жалобу</a>` : ``}
                        ${this['WARNING'] !== 'Y' ? `<a class="color-silver js-avatar-button warning" data-type="warning" data-id="${this['ID']}" data-user="${this['AUTHOR']}">Предупреждение</a>` : ``}
                        <a class="color-silver js-avatar-button ban" data-type="ban" data-id="${this['ID']}" data-user="${this['AUTHOR']}">Бан</a>
                        <a class="color-silver js-avatar-button del-user" data-type="del-user" data-id="${this['ID']}" data-user="${this['AUTHOR']}">Удалить пользователя</a>
                        <a class="color-silver js-avatar-button deactivate" data-type="deactivate" data-id="${this['ID']}" data-user="${this['AUTHOR']}">Удалить жалобу</a>
                        ${this['TICKET'] ? `
    ...
    $("#page").on('click', '.js-avatar-button', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            if (data.status === 'success') {
                ...
                console.log(res)
                if(type === 'del-avatar') {
                    root.find('.js-avatar-action.del').text(res.DEL_CNT)
                    $this.remove()
                    root.find('img.big').attr('src', res.PIC)
                } else if(type === 'reject') {
                    root.find('.js-avatar-action.reject').text(res.REJECT_CNT)
                    $this.remove()
                } else if(type === 'warning') {
                    root.find('.js-avatar-action.warning').text(res.WARNING_CNT)
                    $this.remove()
                } else if(type === 'ban') {
    ...
    $("#page").on('click', '.js-new-chat', function (e) {
        ...
        $.ajax({
            url: '/ajax/chat_avatar_admin.php',
            method: 'POST',
            data: { avatar: id }
        }).done(function (data) {
            data = jQuery.parseJSON(data);
    ...
    $("#page").on('click', '.js-avatar-action', function (e) {
        e.preventDefault();
