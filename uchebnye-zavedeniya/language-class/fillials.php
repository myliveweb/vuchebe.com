...
<div style="text-align: center; margin: -5px auto 25px auto;">
	<span class="color-silver js-new-add" data-vuz="<?=$arResult["ID"]?>" data-type="fillials" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f;">Добавить филиал</span>
</div>
<?php } ?>
<?php
	...
	<div class="line" id="box-line">
		...
		<div class="news-item">
			...
			<div style="position: relative; right: 5px; text-align: right;">
				<div class="color-silver js-news-edit" data-block="fillials" data-id="<?php echo $id_filials; ?>" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f; display: inline-block;">изменить</div>
			</div>
			<?endif?>
			<? if($arrItem[3]) { ?>
			...
			<div class="news-name">
				...
				if($arrItem[1]) {
					...
					$row = $res->Fetch();
				?>
				<a href="/uchebnye-zavedeniya/universities/<?=$row['CODE']?>/" style="color: #ff471a;"><span><?=$arrItem[0]?></span></a>
				<?php } else { ?>
				<span><?=$arrItem[0]?></span>
				<?php } ?>
			</div>
