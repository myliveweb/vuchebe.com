...
<div style="text-align: center; margin: -5px auto 25px auto;">
	<span class="color-silver js-new-add" data-vuz="<?=$arResult["ID"]?>" data-type="programs" data-iblock="2" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f;">Новая программа обучения</span>
</div>
<?php } ?>
<?php
	...
	<div class="module st-news">
		...
		<div class="line" id="box-line">
			<div class="news-item one" style="position: relative;">
				<?if($pageAdmin):?>
				<div class="color-silver js-news-edit" data-block="programs" data-id="<?php echo $id_section; ?>" data-iblock="2" style="position: absolute; top: -6px; right: 5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
				<?endif?>
...
<?php
	...
	foreach($arResult["PROPERTIES"]["PROGRAMS"]["VALUE"] as $sections_id => $sections_item) {
		...
		<div class="news-item<?=$strClass?>" data-filter="<?=$arrItem[7]?>" style="position: relative;">
			<?if($pageAdmin):?>
			<div class="color-silver js-news-edit" data-block="programs" data-id="<?php echo $sections_id; ?>" data-iblock="2" style="position: absolute; right: 5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
			<?endif?>
			<div class="news-name">
