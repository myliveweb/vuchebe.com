...
$arrDelPost = array();
$arrPost = $dbh->query('SELECT post_id from a_group_del_local WHERE chat_id = ' . $groupChat . ' AND user_id = ' . $user_id)->fetchAll();
...
if($mergeArr) {
    ...
    $newPost = $dbh->query('SELECT * from a_chat WHERE group_chat = ' . $groupChat . ' AND id NOT IN(' . $inPost . ') AND del_to = 0 AND owner_id != ' . $user_id)->fetchAll();
} else {
    $newPost = $dbh->query('SELECT * from a_chat WHERE group_chat = ' . $groupChat . ' AND del_to = 0 AND owner_id != ' . $user_id)->fetchAll();
}
foreach($newPost as $item) {
    ...
    if($item['owner_id']) {
        ...
        $arrAdmin = $dbh->query('SELECT user_id from a_group_admin WHERE chat_id = ' . $groupChat)->fetchAll();
...
$arrUser  = $dbh->query('SELECT user_id from a_group_user WHERE chat_id = ' . $groupChat)->fetchAll();
foreach($arrUser as $item) {

    if(CUser::IsOnLine($item['user_id'], 30)) {

        $userOnline = CUser::GetByID($item['user_id']);
        $userChatOnline = $userOnline->Fetch();

        $full_name = trim($userChatOnline['NAME']) . ' ' . trim($userChatOnline['LAST_NAME']);
        if (strlen($full_name) <= 0)
            $full_name = trim($userChatOnline['LOGIN']);

        $userChatOnline['FULL_NAME'] = $full_name;

        if($userChatOnline['PERSONAL_PHOTO']) {
            $userChatOnline['avatar'] = CFile::GetPath($userChatOnline['PERSONAL_PHOTO']);
        } else {
            $userChatOnline['avatar'] = SITE_TEMPLATE_PATH . "/img/foto-user.png";
        }

        $online[] = $userChatOnline;
    }
}

die(json_encode(array("status" => "success", 'update' => $result, 'online' => $online)));
?>