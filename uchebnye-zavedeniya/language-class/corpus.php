...
<div style="text-align: center; margin: -5px auto 25px auto;">
	<span class="color-silver js-new-add" data-vuz="<?=$arResult["ID"]?>" data-type="corpus" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f;">Добавить корпус</span>
</div>
<?php } ?>
<?php
	...
	<div class="line" id="box-line">
		...
		<div class="news-item corpus" data-id="<?=$arResult['ID']?>" data-ukey="<?=$arrItem[10]?>" style="position: relative;">
			...
			<div style="position: relative; top: -10px; right: 5px; text-align: right;">
				<div class="color-silver js-news-edit" data-block="corpus" data-id="<?php echo $id_corpus; ?>" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f; display: inline-block;">изменить</div>
			</div>
			<?endif?>
			<? if($arrItem[4]) { ?>
