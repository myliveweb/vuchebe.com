...
<div style="text-align: center; margin: -5px auto 25px auto;">
	<span class="color-silver js-new-add" data-vuz="<?=$arResult["ID"]?>" data-type="events" data-iblock="2" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f;">Добавить событие</span>
</div>
<?php } ?>
<?php
global $dbh;
...
<div class="page-content" id="page">
	<div class="name-block text-center txt-up"><span>События ВУЗа</span></div>
			...
			<?php
				...
				<div class="news-item events" data-id="<?=$arResult['ID']?>" data-ukey="<?=$itemList['ID']?>">
					...
					<? if($arrItem[1]) {
						...
						$curDate = explode(' ', $strDate);
					?>
					<div class="date-ico" style="margin-bottom: 10px; float: right;"><span><?=$itemList['DAY']?></span><?=$itemList['MONTH']?></div>
					<? } ?>
					<? if($arrItem[4]) { ?>
