...
if($id && $type && $userCurrent && isEdit()) {
    ...
    if($type === 'del-reviews') {
        ...
        $arSelectPost = array("ID", "NAME", "IBLOCK_ID");
        $arFilterPost = array("IBLOCK_ID" => 23, "ACTIVE" => "Y", "ID" => $id);
        $resPost = CIBlockElement::GetList(array("ID" => "ASC"), $arFilterPost, false, false, $arSelectPost);
        if($obResPost = $resPost->GetNextElement()) {

            $propsPost = $obResPost->GetProperties();
            $postId   = $propsPost['POST_ID']['VALUE'];

            CIBlockElement::Delete($postId);

        }

        CIBlockElement::SetPropertyValueCode($id, "DEL", 'Y');

        $arFilterDel = Array("IBLOCK_ID" => 23, "ACTIVE" => "Y", "PROPERTY_AUTHOR" => $userCurrent, "PROPERTY_DEL" => "Y");
        ...
        $arFilterRej = Array("IBLOCK_ID" => 23, "ACTIVE" => "Y", "PROPERTY_AUTHOR" => $userCurrent, "PROPERTY_REJECT" => "Y");
        ...
        $arFilterWarning = Array("IBLOCK_ID" => 23, "ACTIVE" => "Y", "PROPERTY_AUTHOR" => $userCurrent, "PROPERTY_WARNING" => "Y");
        ...
        $arFilter = array("IBLOCK_ID" => 23, "PROPERTY_AUTHOR" => $userCurrent);
    ...
    $arFilterCntNew = Array("IBLOCK_ID" => 23, "ACTIVE" => "Y", "!PROPERTY_WARNING" => "Y", "!PROPERTY_REJECT" => "Y", "!PROPERTY_DEL" => "Y");
    $resCntNew = CIBlockElement::GetList(array(), $arFilterCntNew, Array(), false, Array());
    $result['NEW'] = $resCntNew ? $resCntNew : 0;

    $arFilterCntRej = Array("IBLOCK_ID" => 23, "ACTIVE" => "Y", "PROPERTY_REJECT" => "Y");
    $resCntRej = CIBlockElement::GetList(array(), $arFilterCntRej, Array(), false, Array());
    $result['REJECT'] = $resCntRej ? $resCntRej : 0;

    $arFilterCntWarning = Array("IBLOCK_ID" => 23, "ACTIVE" => "Y", "PROPERTY_WARNING" => "Y");
