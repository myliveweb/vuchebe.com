...
<div style="text-align: center; margin: -5px auto 25px auto;">
	<span class="color-silver js-new-add" data-vuz="<?=$arResult["ID"]?>" data-type="units" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f;">Добавить подразделение</span>
</div>
<?php } ?>
<?php
		...
		<div class="line" id="box-line">
			<div class="news-item one">
				...
				<div style="position: relative; top: -10px; right: 5px; text-align: right;">
					<div class="color-silver js-news-edit" data-block="units" data-id="<?php echo $id_section; ?>" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f; display: inline-block;">изменить</div>
				</div>
				<?endif?>
				<? if($section[5]) { ?>
...
<?php
	...
	<div class="news-item">
		...
		<?if($pageAdmin):?>
		<div style="position: relative; top: -10px; right: 5px; text-align: right;">
			<div class="color-silver js-news-edit" data-block="units" data-id="<?php echo $sections_id; ?>" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f; display: inline-block;">изменить</div>
		</div>
		<?endif?>
		<? if($arrItem[5]) { ?>
			<div class="btns text-right"><a href="#" class="button"><span style="font-family: Verdana;">на карте</span></a></div>
		...
		<div class="news-name">
			<a href="<?=$arResult["DETAIL_PAGE_URL"]?>?sect=units&s=<?php echo ($sections_id + 1); ?>"><span><?=$arrItem[0]?></span></a>
		</div>
		<p>
		<?php
			for($n = 1; $n < sizeof($placeholder); $n++) {
