...
require($_SERVER["DOCUMENT_ROOT"].'/include/left_menu_course.php');
require($_SERVER["DOCUMENT_ROOT"].'/include/svg_short.php');
//echo '<pre>';
//print_r($arResult);
//echo '</pre>';
global $dbh;
...
<div class="st-content-right">
	<div class="breadcrumbs" style="margin-bottom: 15px;">
		<a href="/">Главная</a> <i class="fa fa-angle-double-right color-orange"></i> <a href="/uchebnye-zavedeniya/">Учебные зведения</a> <i class="fa fa-angle-double-right color-orange"></i> <a href="/uchebnye-zavedeniya/language-class/">Языковые курсы</a> <i class="fa fa-angle-double-right color-orange"></i> <?php echo mb_ucfirst($arResult["NAME"]); ?>
	</div>
	<?
	if($structure_menu):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/language-class/structure.php');
	elseif($_REQUEST['sect'] == 'history'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/language-class/history.php');
	elseif($_REQUEST['sect'] == 'teacher'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/language-class/teacher.php');
	elseif($_REQUEST['sect'] == 'students'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/language-class/students.php');
	elseif($_REQUEST['sect'] == 'opendoor'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/language-class/opendoor.php');
	elseif($_REQUEST['sect'] == 'events'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/language-class/events.php');
	elseif($_REQUEST['sect'] == 'news'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/language-class/news.php');
	elseif($_REQUEST['sect'] == 'reviews'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/language-class/reviews.php');
	else:
	?>
		...
		<div class="page-item clearfix">
			<div class="col-3 content-left">
				...
				if($arResult["PROPERTIES"]["LOGO"]["VALUE"]):
					...
					$altLogo = 'Логотип языкового курса';
					$titleLogo = 'Логотип языкового курса';
				elseif(is_array($arResult["DETAIL_PICTURE"])):
					$srcLogo = $arResult["DETAIL_PICTURE"]["SRC"];
					$altLogo = $arResult["DETAIL_PICTURE"]["ALT"];
					$titleLogo = $arResult["DETAIL_PICTURE"]["TITLE"];
				else:
				...
				if($pageAdmin):?>
					<input type="file" id="file" data-type="logo" data-id="<?php echo $arResult["ID"]; ?>" data-iblock="6" accept="image/*">
					<label for="file">
				...
				<div class="btns" style="margin-top: 10px;">
					...
					<a href="#" class="button js-bookmark active" data-state="1" data-type="4" data-id="<?php echo $arResult["ID"]; ?>">
						<span>закладки</span>
					</a>
					<?php } else { ?>
					<a href="#" class="button js-bookmark" data-state="0" data-type="4" data-id="<?php echo $arResult["ID"]; ?>">
						<span>закладки</span>
					</a>
					<?php } ?>
					<?php if($arResult["PROPERTIES"]["YANDEX"]["VALUE"]) { ?>
			...
			<div class="col-9 content-right">
				<div class="page-info">
					...
					<div class="contact-info" style="position: relative;">
						<?if($pageAdmin):?>
						<div class="color-silver js-vuz-edit" data-block="first" data-iblock="6" style="position: absolute; top: 0px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
						<?endif?>
						<?if($arResult["SECTION"]["PATH"][0]):?>
						<!--<span>Город: <a href="<?=$arResult["SECTION"]["PATH"][0]["SECTION_PAGE_URL"]?>"><?=$arResult["SECTION"]["PATH"][0]["NAME"]?></a></span>-->
						<?endif?>
						<?
						...
						<?endif?>
						<?if(sizeof($arResult["CITY_DOP"]) > 1):?>
						<span>Школа в других городах: </span>
						<div style="margin-bottom: 15px;">
							<ul style="list-style: none; padding-left: 10px; margin-bottom: 30px;">
							<?
							foreach ($arResult["CITY_DOP"] as $item_city) {
							?>
								<li style="margin-bottom: 5px;">
									<a href="<?=$item_city["URL"]?>" alt="<?=$item_city["NAME"]?>" title="<?=$item_city["NAME"]?>"><?=$item_city["NAME"]?></a>
								</li>
							<?
							}
							?>
						...
						<?endif?>
						<?if(sizeof($arResult["PROPERTIES"]["ADRESS"]["VALUE"]) > 1) { ?>
	                        <span style="margin-top: 4px;">Адреса филлиалов: <div id="show-filials" style="display: inline-block; cursor: pointer; border-bottom: 1px dashed #9f9f9f; color: #9f9f9f;">показать</div></span>
							<div class="school-card__advantages adress" id="js-filial-adress" style="display: none; padding-left: 15px;">
							    <ul class="school-advantages-list" style="padding-left: 0px;">
								<?foreach ($arResult["PROPERTIES"]["ADRESS"]["VALUE"] as $item_adress) {
									$arrAdress = explode('&', $item_adress);
								?>
	                                <li style="margin-bottom: 0px;" data-geo="<?=$arrAdress[1]?>" class="school-advantages-list__item">
	                                    <span><?=$arrAdress[0]?></span>
	                                </li>
	                            <? } ?>
								</ul>
	                        </div>
                        <? } elseif(sizeof($arResult["PROPERTIES"]["ADRESS"]["VALUE"]) == 1) {
                        	$arrAdress = explode('&', $arResult["PROPERTIES"]["ADRESS"]["VALUE"][0]);
                        ?>
							<span style="margin-top: 4px;">Адрес филлиала: <?=$arrAdress[0]?></span>
                        <? } ?>
						...
						<?endif?>
						<?if($arResult["PROPERTIES"]["ELECTRON_PR"]["VALUE"]): ?>
						<span><?=$arResult["PROPERTIES"]["ELECTRON_PR"]["NAME"]?>: <a href="<?=$arResult["PROPERTIES"]["ELECTRON_PR"]["VALUE"]?>" target="_blank"><?=$arResult["PROPERTIES"]["ELECTRON_PR"]["VALUE"]?></a></span>
						<?endif?>
						<?if($arResult["PROPERTIES"]["SITE"]["VALUE"]):
							$arrSite = explode('?', $arResult["PROPERTIES"]["SITE"]["VALUE"]);
						?>
						<span><?=$arResult["PROPERTIES"]["SITE"]["NAME"]?>: <a href="<?=$arrSite[0]?>" target="_blank"><?=$arrSite[0]?></a></span>
						<?endif?>

						<?if($arResult["PROPERTIES"]["COST_HOUR"]["VALUE"]):?>
						<span>Стоимость час: <span style="font-weight: bold; display: inline-block;"><?=$arResult["PROPERTIES"]["COST_HOUR"]["VALUE"]?></span> руб.</span>
						<?endif?>
						<?if($arResult["PROPERTIES"]["COST_MONTH"]["VALUE"]):?>
						<span>Стоимость месяц: <span style="font-weight: bold; display: inline-block;"><?=$arResult["PROPERTIES"]["COST_MONTH"]["VALUE"]?></span> руб.</span>
						<?endif?>

						<?if($arResult["PROPERTIES"]["LICENCE"]["VALUE"]
						 || $arResult["PROPERTIES"]["TESTLESSON"]["VALUE"]
						 || $arResult["PROPERTIES"]["GROUP_CURS"]["VALUE"]
						 || $arResult["PROPERTIES"]["CHILDREN"]["VALUE"]
						 || $arResult["PROPERTIES"]["PAYMENT"]["VALUE"]):?>
						 <div style="margin-top: 10px;">
					    	<?if($arResult["PROPERTIES"]["LICENCE"]["VALUE"]):?>
                                <span><?=$arResult["PROPERTIES"]["LICENCE"]["VALUE"]?></span>
                            <?endif?>

							<?if($arResult["PROPERTIES"]["TESTLESSON"]["VALUE"]):?>
                                <span><?=$arResult["PROPERTIES"]["TESTLESSON"]["VALUE"]?></span>
                            <?endif?>

							<?if($arResult["PROPERTIES"]["GROUP_CURS"]["VALUE"]):?>
                                <span><?=$arResult["PROPERTIES"]["GROUP_CURS"]["VALUE"]?></span>
                            <?endif?>

							<?if($arResult["PROPERTIES"]["CHILDREN"]["VALUE"]):?>
                                <span><?=$arResult["PROPERTIES"]["CHILDREN"]["VALUE"]?></span>
                            <?endif?>
						...
						<div class="links" style="position: relative;">
							<?if($pageAdmin):?>
							<div class="color-silver js-vuz-edit" data-block="soc" data-iblock="6" style="position: absolute; top: 0px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
							<?endif?>
							<?if($arResult["PROPERTIES"]["VK"]["VALUE"]):?>
						...
						<div class="links" style="position: relative; margin-bottom: 60px;">
							<div class="color-silver js-vuz-edit" data-block="soc" data-iblock="6" style="position: absolute; top: 0px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">добавить ссылки</div>
						</div>
						<? } ?>
						<div style="position: relative;">
							<?if($pageAdmin):?>
							<div class="color-silver js-vuz-edit" data-block="license" data-iblock="6" style="position: absolute; top: -5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
							<?endif?>
							...
							<?if($arResult["PROPERTIES"]["FIO_RUKOVODSTVO"]["VALUE"]):?>
							<div class="title"><b>Руководитель:</b></div>
							<span><?=$arResult["PROPERTIES"]["FIO_RUKOVODSTVO"]["VALUE"]?></span>
							<?endif?>
						</div>
					</div><!-- contact-info -->

					<? if($arResult["PROPERTIES"]["YEAR"]) {
			...
			<div class="st-content-bottom clear" style="margin-top: 15px;">
				...
				<div class="category-ico" style="position: relative;">
					<?if($pageAdmin):?>
						<div class="color-silver js-vuz-edit" data-block="service" data-iblock="6" style="position: absolute; top: -28px; right: 5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
					<?endif?>
				...
				<div class="category-ico" style="position: relative;">
					<div class="color-silver js-vuz-edit" data-block="service" data-iblock="6" style="position: absolute; top: -28px; right: 5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">добавить сервисы</div>
				</div>
				<? }
				...
				<div class="module st-news">
					...
					foreach($newsList as $ts => $itemList) {
						...
						<div class="news-item news" style="position: relative;">
							<?if($pageAdmin):?>
							<div class="color-silver js-news-edit" data-block="news" data-id="<?php echo $itemList["ID"]; ?>" data-iblock="6" style="position: absolute; right: 5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
							<?endif?>
						...
						<div class="news-item opendoor" style="display: none;">
							...
							<?if($pageAdmin):?>
							<div style="position: relative; top: -10px; right: 5px; text-align: right;">
								<div class="color-silver js-news-edit" data-block="opendoor" data-id="<?php echo $itemList["ID"]; ?>" data-iblock="6" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f; display: inline-block;">изменить</div>
							</div>
							<?endif?>
							<? if($arrItem[1]) { ?>
							<div class="date-ico" style="margin-bottom: 10px;"><span><?=$itemList['DAY']?></span><?=$itemList['MONTH']?></div>
