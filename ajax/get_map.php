...
if(in_array(6, $input['vuzId'])) {
	...
	$res = CIBlockElement::GetList(array("ID" => "ASC"), $arFilter, array("ID"), false, $arSelect);
	while($row = $res->Fetch()) {

		$arSelectOne = array("ID", "NAME", "IBLOCK_ID", "CODE", "PREVIEW_PICTURE", "DETAIL_PICTURE", "PROPERTY_LOGO", "PROPERTY_ADRESS", "PROPERTY_LONGITUDE", "PROPERTY_LATITUDE", "PROPERTY_PHONE", "PROPERTY_CITY");
		$arFilterOne = array("IBLOCK_ID" => 6, "ACTIVE" => "Y", "ID" => $row['ID']);
		$resOne = CIBlockElement::GetList(array("ID" => "ASC"), $arFilterOne, false, false, $arSelectOne);
		if($rowOne = $resOne->GetNext()) {

			if($rowOne["PROPERTY_LOGO_VALUE"]):
				$srcLogo = CFile::GetPath($rowOne["PROPERTY_LOGO_VALUE"]);
			elseif($rowOne["PREVIEW_PICTURE"]):
				$srcLogo = CFile::GetPath($rowOne["PREVIEW_PICTURE"]);
			elseif($rowOne["DETAIL_PICTURE"]):
				$srcLogo = CFile::GetPath($rowOne["DETAIL_PICTURE"]);
			else:
				$srcLogo = SITE_TEMPLATE_PATH . '/images/noimage.png';
			endif;

			$url = 'http://vuchebe.com/uchebnye-zavedeniya/language-class/' . $rowOne["CODE"] . '/';

		    $resGeo = CIBlockElement::GetProperty($rowOne["IBLOCK_ID"], $rowOne["ID"], "sort", "asc", array("CODE" => "YANDEX"));
		    while($obGeo = $resGeo->GetNext())
		    {
		    	if($obGeo['VALUE']) {
		    		$arrGeo = explode('#', $obGeo['VALUE']);

		    		if($rowOne["PROPERTY_CITY_VALUE"]) {
		    			if(stristr($arrGeo[0], $rowOne["PROPERTY_CITY_VALUE"]) === false) {
		    				$adress = $rowOne["PROPERTY_CITY_VALUE"] . ', ' . $arrGeo[0];
		    			} else {
		    				$adress = $arrGeo[0];
		    			}
		    		}
		    		...
		    		if(is_numeric($arrGeo[2]) && is_numeric($arrGeo[1])) {
						...
						$geo[] = array(preg_replace('/[^a-zA-Zа-яА-Я0-9()+.,\/ -]/ui', '', $adress),
							$arrGeo[2],
							$arrGeo[1],
							preg_replace('/[^a-zA-Zа-яА-Я0-9()+.,\/ -]/ui', '', $rowOne["NAME"]),
							$url,
							preg_replace('/[^0-9()+ -]/', '', $rowOne["PROPERTY_PHONE_VALUE"]),
							$srcLogo,
							'bal-4.png'
						);
					}
