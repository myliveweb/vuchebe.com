...
$arResult["STUDENTS"] = $dbh->query('SELECT * from a_user_uz WHERE type = 3 AND teacher = 0 AND uz_id = ' . $arResult["ID"] . ' ORDER BY user_name ASC')->fetchAll();
...
$arResult["TEACHER"] = $dbh->query('SELECT * from a_user_uz WHERE type = 3 AND teacher = 1 AND uz_id = ' . $arResult["ID"] . ' ORDER BY user_name ASC')->fetchAll();
...
if($user_id) {
	$arResult["BOOKMARK"] = $dbh->query('SELECT * from a_bookmark WHERE type = 3 AND uz_id = ' . $arResult["ID"] . ' AND user_id = ' . $user_id)->fetch();
}
