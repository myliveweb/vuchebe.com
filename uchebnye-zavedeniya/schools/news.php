...
<div style="text-align: center; margin: -5px auto 25px auto;">
	<span class="color-silver js-new-add" data-vuz="<?=$arResult["ID"]?>" data-type="news" data-iblock="4" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f;">Добавить новость</span>
</div>
<?php } ?>
<?php
global $dbh;
...
if($_REQUEST['s']) {
	...
	$arFilter = array("IBLOCK_ID" => 29, "ACTIVE" => "Y", "ID" => $id_news);
...
<div class="name-block text-center txt-up"><span>Новости школы</span></div>
<div class="st-content-bottom clear">
	<div class="module st-news">
		<div class="line" id="box-line" data-url="<?=$arResult["DETAIL_PAGE_URL"]?>?sect=news">
			...
			<div class="news-item one" style="position: relative;">
				<?if($pageAdmin):?>
				<div class="color-silver js-news-edit" data-block="news" data-id="<?php echo $news_item["ID"]; ?>" data-iblock="4" style="position: absolute; right: 5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
				<?endif?>
	...
	<div class="st-carousel news-3">
		<div class="owl-carousel">
			...
			$arFilter = array("IBLOCK_ID" => 29, "ACTIVE" => "Y", "PROPERTY_VUZ_ID" => $arResult["ID"], "!ID" => $news_item["ID"]);
...
<?php
	...
	echo '<div class="name-block text-center txt-up"><span>Новости школы</span></div>';
	...
	if($nNews) {
		echo '<div class="line" id="box-line" data-vuz="' . $arResult['ID'] . '" data-type="news">';
		for($n = 0; $n < $nNews; $n++) {
			$news_item = $arrNews[$n];
		?>
		<div class="news-item news" style="position: relative;">
			...
			<?php
				...
				foreach($deslike_news_cnt[$news_item['ID']] as $news_item_baloon) {
					if($en >= $showBaloon) {
						echo '<div class="more-baloon"><span data-id-vuz="' . $arResult['ID'] . '" data-type="news" data-id="' . $news_item["ID"] . '" data-hash="deslike" style="margin-left: 10px; font-size: 10px; top: 12px; position: relative;">ещё</span></div>';
						break;
					} else {
						$en++;
						$rsUserData = CUser::GetByID($news_item_baloon["id_user"]);
