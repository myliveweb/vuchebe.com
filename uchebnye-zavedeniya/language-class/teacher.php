...
$arrFilter = array();
$flagArray = $dbh->query('SELECT * from a_user_uz WHERE uz_id = ' . $arResult['ID'] . ' AND teacher = 1 ORDER BY user_name ASC')->fetchAll();
...
<div class="page-content st-content-users" id="page">
    ...
    <!-- End Поиск -->
	<div class="name-block text-center txt-up"><span>Преподаватели языкового курса</span></div>
	...
	<? if($arResult["TEACHER"]) { ?>
		<div class="module st-news">
			<div class="m-header" style="padding-bottom: 10px;">
				...
				<?if($arrFilter['start']):?>
				<a href="#" data-filter="start" class="filter js-user-list">Преподают</a> &nbsp;
				<?endif?>
				<?if($arrFilter['end']):?>
				<a href="#" data-filter="end" class="filter js-user-list">Преподавали</a> &nbsp;
				<?endif?>
			</div>
			<div class="line" id="box-line" data-type="students">
				...
				foreach($arResult["TEACHER"] as $item) {
		            ...
		            $flag = $dbh->query('SELECT * from a_user_uz WHERE uz_id = ' . $arResult['ID'] . ' AND user_id = ' . $arResult['USER']['ID'] . ' AND teacher = 1')->fetch();
				...
				<div class="news-item<?php if($flag['end_p']) { echo ' end'; } elseif($flag['start_p']) { echo ' start'; } ?>">
					<div class="col-3 width-sm content-left">
						<div class="image brd rad-50"><img src="<?=$avatar_url?>" alt="img" style="height: 111px; width: 111px; border: 3px solid #ff5b32;"></div>
						<br>
					</div>
	                <div class="col-9 width-sm content-right" style="padding: 0;">
	                    <div class="page-info">
	                        ...
	                        <div class="contact-info">
								...
								if($flag['end_p']) {
								?>
								<div class="date-ico theme-1 right" style="background: url(/local/templates/vuchebe/images/stick-year-3.png) no-repeat; position: relative;">
									преподавал<br>до
