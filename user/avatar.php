...
$arSelect = array("ID", "NAME", "IBLOCK_ID", "DATE_CREATE", "DETAIL_PICTURE");
...
while($obRes = $res->GetNextElement()) {
    ...
    if(sizeof($outArray) < $cnt && $row['WARNING'] != 'Y' && $row['REJECT'] != 'Y' && $row['DEL'] != 'Y') {
        ...
        if($row["DETAIL_PICTURE"]) {
            $row["PIC"] = CFile::GetPath($row["DETAIL_PICTURE"]);
        } else {
        ...
        $row["TOTAL"] = 0;
        $arFilter = Array("IBLOCK_ID"=>27, "ACTIVE"=>"Y", "PROPERTY_AUTHOR" => $row["AUTHOR"]);
        ...
        $row["WARNING_CNT"] = 0;
        $arFilterWarning = Array("IBLOCK_ID"=>27, "ACTIVE"=>"Y", "PROPERTY_AUTHOR" => $row["AUTHOR"], "PROPERTY_WARNING" => "Y");
        ...
        $row["REJECT_CNT"] = 0;
        $arFilterReject = Array("IBLOCK_ID"=>27, "ACTIVE"=>"Y", "PROPERTY_AUTHOR" => $row["AUTHOR"], "PROPERTY_REJECT" => "Y");
        ...
        $row["DEL_CNT"] = 0;
        $arFilterDel = Array("IBLOCK_ID"=>27, "ACTIVE"=>"Y", "PROPERTY_AUTHOR" => $row["AUTHOR"], "PROPERTY_DEL" => "Y");
        ...
        $row["TICKET_CNT"] = 0;
        $arFilterTicket = Array("IBLOCK_ID"=>27, "ACTIVE"=>"Y", "PROPERTY_AUTHOR" => $row["AUTHOR"], "!PROPERTY_TICKET" => false);
...
<div class="st-content-right">
	<div class="breadcrumbs">
		<a href="/">Главная</a> <i class="fa fa-angle-double-right color-orange"></i> <a href="/user/<?php echo $url; ?>/">Профиль</a> <i class="fa fa-angle-double-right color-orange"></i> <span>Аватар</span>
	</div><br>
    <div class="page-content st-content-users" id="page">
        ...
        <!-- End Поиск -->
		<div class="name-block text-center txt-up" style="margin: 0 0 15px;"><span>Аватар</span></div>
		<div class="st-content-bottom clear">
			<div class="module st-news">
                <?php if(!$search) { ?>
				<div class="m-header">
                    <a href="#" data-filter="new" class="filter js-avatar-list color-silver">новые(<span class="js-new"><?php if($arrFilter['new']) { echo $arrFilter['new']; } else { echo '0'; } ?></span>)</a>
					<a href="#" data-filter="otklon" class="filter js-avatar-list">отклоненные(<span class="js-otklon"><?php if($arrFilter['otklon']) { echo $arrFilter['otklon']; } else { echo '0'; } ?></span>)</a>
					<a href="#" data-filter="warning" class="filter js-avatar-list">предупреждения(<span class="js-warning"><?php if($arrFilter['warning']) { echo $arrFilter['warning']; } else { echo '0'; } ?></span>)</a>
					<a href="#" data-filter="del" class="filter js-avatar-list">удалённые аватары(<span class="js-del"><?php if($arrFilter['del']) { echo $arrFilter['del']; } else { echo '0'; } ?></span>)</a>
                    <a href="#" data-filter="all" class="filter js-avatar-list">все(<span class="js-all"><?php if($arrFilter['all']) { echo $arrFilter['all']; } else { echo '0'; } ?></span>)</a>
				</div>
                <?php } else { ?>
                    <div class="search-result">
                        <div class="result-left">Найдено: <?php echo sizeof($outArray); ?></div>
                        <div class="result-right"><a href="/user/<?php echo $url; ?>/avatar/">назад к списку аватаров</a></div>
                    </div>
                <?php } ?>
        ...
        <div class="news-item" data-id="<?php echo $arrData['ID']; ?>">
          ...
          <div class="col-9 width-sm content-right">
            <div class="news-name">
                <span><a href="/user/<?=$arrData['URL']?>/" class="display-name" target="blank"><?=$arrData['FORMAT_NAME']?></a></span>
            </div>
            <div style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">
                ...
                <div class="params-banner">Предыдущие жалобы: <a href="#" class="js-avatar-action abuse" data-id="<?=$arrData['ID']?>" data-user="<?=$arrData['AUTHOR']?>" data-type="abuse"><?php echo $arrData["TOTAL"]; ?></a></div>
                <div class="params-banner">Предупреждений: <a href="#" class="js-avatar-action warning" data-id="<?=$arrData['ID']?>" data-user="<?=$arrData['AUTHOR']?>" data-type="warning"><?php echo $arrData["WARNING_CNT"]; ?></a></div>
                <div class="params-banner">Отклонённые жалобы: <a href="#" class="js-avatar-action reject" data-id="<?=$arrData['ID']?>" data-user="<?=$arrData['AUTHOR']?>" data-type="reject"><?php echo $arrData["REJECT_CNT"]; ?></a></div>
                <div class="params-banner">Удаление аватара: <a href="#" class="js-avatar-action del" data-id="<?=$arrData['ID']?>" data-user="<?=$arrData['AUTHOR']?>" data-type="del"><?php echo $arrData["DEL_CNT"]; ?></a></div>
                <div class="params-banner">Созданных тикетов: <a href="#" class="js-avatar-action ticket" data-id="<?=$arrData['ID']?>" data-user="<?=$arrData['AUTHOR']?>" data-type="ticket"><?php echo $arrData["TICKET_CNT"]; ?></a></div>
            </div>
          </div>
        <div class="params-banner-top col-12" style="margin-top: 15px; text-align: right;">
            <a class="color-silver js-avatar-button del-avatar" data-type="del-avatar" data-id="<?php echo $arrData["ID"]; ?>" data-user="<?=$arrData['AUTHOR']?>">Удалить аватар</a>
            <a class="color-silver js-avatar-button reject" data-type="reject" data-id="<?php echo $arrData["ID"]; ?>" data-user="<?=$arrData['AUTHOR']?>">Отклонить жалобу</a>
            <a class="color-silver js-avatar-button warning" data-type="warning" data-id="<?php echo $arrData["ID"]; ?>" data-user="<?=$arrData['AUTHOR']?>">Предупреждение</a>
            <a class="color-silver js-avatar-button ban" data-type="ban" data-id="<?php echo $arrData["ID"]; ?>" data-user="<?=$arrData['AUTHOR']?>">Бан</a>
            <a class="color-silver js-avatar-button del-user" data-type="del-user" data-id="<?php echo $arrData["ID"]; ?>" data-user="<?=$arrData['AUTHOR']?>">Удалить пользователя</a>
            <a class="color-silver js-avatar-button deactivate" data-type="deactivate" data-id="<?php echo $arrData["ID"]; ?>" data-user="<?=$arrData['AUTHOR']?>">Удалить жалобу</a>
