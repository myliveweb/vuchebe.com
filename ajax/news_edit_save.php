...
if($isAdmin) {
	if($input['type'] == 'news') {
		...
		if($input['name']) {
			...
			$result = $el->Update($input['id_block'], $arLoadProductArray);
			...
			{
				CIBlockElement::SetPropertyValueCode($input['id_block'], "COUNTRY", $rowCarusel['PROPERTY_COUNTRY_VALUE']);
				CIBlockElement::SetPropertyValueCode($input['id_block'], "REGION", $rowCarusel['PROPERTY_REGION_VALUE']);
				CIBlockElement::SetPropertyValueCode($input['id_block'], "CITY", $rowCarusel['PROPERTY_CITY_VALUE']);
			}
