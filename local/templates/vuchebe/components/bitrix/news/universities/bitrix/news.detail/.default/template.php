...
require($_SERVER["DOCUMENT_ROOT"].'/include/left_menu_universities.php');
//echo '<pre>';
//print_r($arResult["PROPERTIES"]["DOP_LINK"]["VALUE"]);
//echo '</pre>';
global $dbh;
...
<div class="st-content-right">
	<div class="breadcrumbs" style="margin-bottom: 20px;">
		<a href="/">Главная</a> <i class="fa fa-angle-double-right color-orange"></i> <a href="/uchebnye-zavedeniya/">Учебные зведения</a> <i class="fa fa-angle-double-right color-orange"></i> <a href="/uchebnye-zavedeniya/universities/">ВУЗы</a> <i class="fa fa-angle-double-right color-orange"></i> <?php echo mb_ucfirst($arResult["NAME"]); ?>
	</div>
	<?if($structure_menu):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/universities/structure.php');
	elseif($_REQUEST['sect'] == 'history'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/universities/history.php');
	elseif($_REQUEST['sect'] == 'teacher'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/universities/teacher.php');
	elseif($_REQUEST['sect'] == 'students'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/universities/students.php');
	elseif($_REQUEST['sect'] == 'opendoor'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/universities/opendoor.php');
	elseif($_REQUEST['sect'] == 'events'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/universities/events.php');
	elseif($_REQUEST['sect'] == 'news'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/universities/news.php');
	elseif($_REQUEST['sect'] == 'reviews'):
		require($_SERVER["DOCUMENT_ROOT"].'/uchebnye-zavedeniya/universities/reviews.php');
	else:
	?>
		...
		<div class="page-item clearfix">
			<div class="col-3 content-left">
				...
				if($arResult["PROPERTIES"]["LOGO"]["VALUE"]):
					...
					$altLogo = 'Логотип ВУЗа';
					$titleLogo = 'Логотип ВУЗа';
				...
				if($pageAdmin):?>
					<input type="file" id="file" data-type="logo" data-id="<?php echo $arResult["ID"]; ?>" data-iblock="2" accept="image/*">
					<label for="file">
				...
				<div class="btns" style="margin-top: 10px;">
					...
					<a href="#" class="button js-bookmark active" data-state="1" data-type="1" data-id="<?php echo $arResult["ID"]; ?>">
						<span>закладки</span>
					</a>
					<?php } else { ?>
					<a href="#" class="button <?php if($_SESSION['USER_DATA']) { echo 'js-bookmark'; } else { echo 'js-noauth'; } ?>" data-state="0" data-type="1" data-id="<?php echo $arResult["ID"]; ?>">
						<span>закладки</span>
					</a>
					<?php } ?>
			...
			<div class="col-9 content-right">
				<div class="page-info">
					...
					<div class="contact-info" style="position: relative;">
						<?if($pageAdmin):?>
						<div class="color-silver js-vuz-edit" data-block="first" data-iblock="2" style="position: absolute; top: 0px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
						<?endif?>
						<?
						...
						<div class="links" style="position: relative;">
							<?if($pageAdmin):?>
							<div class="color-silver js-vuz-edit" data-block="soc" data-iblock="2" style="position: absolute; top: 0px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
							<?endif?>
							<?if($arResult["PROPERTIES"]["VK"]["VALUE"]):?>
						...
						<div class="links" style="position: relative; margin-bottom: 60px;">
							<div class="color-silver js-vuz-edit" data-block="soc" data-iblock="2" style="position: absolute; top: 0px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">добавить ссылки</div>
						</div>
						<? } ?>
						<div style="position: relative;">
							<?if($pageAdmin):?>
							<div class="color-silver js-vuz-edit" data-block="license" data-iblock="2" style="position: absolute; top: -5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
							<?endif?>
			...
			<div class="st-content-bottom clear" style="margin-top: 15px;">
				...
				<div class="category-ico" style="position: relative;">
					<?if($pageAdmin):?>
						<div class="color-silver js-vuz-edit" data-block="service" data-iblock="2" style="position: absolute; top: -28px; right: 5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
					<?endif?>
				...
				<div class="category-ico" style="position: relative;">
					<div class="color-silver js-vuz-edit" data-block="service" data-iblock="2" style="position: absolute; top: -28px; right: 5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">добавить сервисы</div>
				</div>
				<? }
				...
				if($arResult["NEWS"]) {
					...
					$arFilter = array("IBLOCK_ID" => 22, "ACTIVE" => "Y", "PROPERTY_VUZ_ID" => $arResult["ID"]);
				...
				<div class="module st-news">
					...
					foreach($newsList as $ts => $itemList) {
						...
						<div class="news-item news" style="position: relative;">
							<?if($pageAdmin):?>
							<div class="color-silver js-news-edit" data-block="news" data-id="<?php echo $itemList["ID"]; ?>" data-iblock="2" style="position: absolute; right: 5px; cursor: pointer; border-bottom: 1px dashed #9f9f9f;">изменить</div>
							<?endif?>
						...
						<div class="news-item events" data-id="<?=$arResult['ID']?>" data-ukey="<?=$itemList['ID']?>" style="display: none;">
							...
							<div style="position: relative; top: -10px; right: 5px; text-align: right;">
								<div class="color-silver js-news-edit" data-block="events" data-id="<?php echo $itemList["ID"]; ?>" data-iblock="2" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f; display: inline-block;">изменить</div>
							</div>
							<?endif?>
							<? if($arrItem[1]) { ?>
							...
							<div class="btns text-right" style="text-align: left;">
								<a href="/map/?map=<?php echo $arResult["ID"]; ?>&event=<?php echo $itemList["ID"]; ?>" class="button">
							...
							<? } ?>
								...
								<div class="btns text-right" style="margin-top: 9px; text-align: left; position: relative;">
									...
									<?php } ?>
									<a href="#" data-lk="0" class="button <?php if($_SESSION['USER_DATA']) { echo 'js-event-go'; } else { echo 'js-noauth'; } ?> b-right<?php if(in_array($itemList['ID'], $deslike_events_go)) { echo " active"; } ?>" style="position: relative; right: 0px; top: 0px;">
										<span style="text-decoration: none;">Я пойду (<?php echo sizeof($deslike_events_go_cnt[$itemList['ID']]); ?>)</span>
									</a>
								</div>
							</div>
						...
						<div class="news-item opendoor" style="display: none;">
							...
							<?if($pageAdmin):?>
							<div style="position: relative; top: -10px; right: 5px; text-align: right;">
								<div class="color-silver js-news-edit" data-block="opendoor" data-id="<?php echo $itemList["ID"]; ?>" data-iblock="2" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f; display: inline-block;">изменить</div>
							</div>
							<?endif?>
							<? if($arrItem[1]) { ?>
							<div class="date-ico" style="margin-bottom: 10px;"><span><?=$itemList['DAY']?></span><?=$itemList['MONTH']?></div>
