...
if($user_id) {
	...
	if($input['status']) {
		$count = $dbh->exec('DELETE FROM ' . $tab . ' WHERE id_user = ' . $user_id . ' AND id_vuz = ' . $input['id_vuz'] . ' AND id_news = ' . $input['id_news']);
		$cnt = $input['cnt'] - 1;
	} else {
		$stmt = $dbh->prepare("INSERT INTO " . $tab . " (id_user, id_vuz, id_news, user_name, user_avatar) VALUES (:id_user, :id_vuz, :id_news, :user_name, :user_avatar)");
		$stmt->bindParam(':id_user', $user_id);
		$stmt->bindParam(':id_vuz', $input['id_vuz']);
		$stmt->bindParam(':id_news', $input['id_news']);
		$stmt->bindParam(':user_name', $user_name);
		$stmt->bindParam(':user_avatar', $_SESSION['USER_DATA']['AVATAR']);
		$stmt->execute();
	}

	$users = $dbh->query('SELECT * from ' . $tab . ' WHERE id_vuz = ' . $input['id_vuz'] . ' AND id_news = ' . $input['id_news'] . ' ORDER BY id DESC')->fetchAll();
}
