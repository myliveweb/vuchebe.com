...
<div style="text-align: center; margin: 5px auto 20px auto;">
	<span class="color-silver js-admins-add" data-id="0" data-vuz-id="<?php echo $arResult['ID']; ?>" data-iblock="2" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f;">Добавить администратора</span>
</div>
<?php } ?>
<style>
.display-name {
...
<div class="module st-news">
	<div class="line" id="box-line" data-type="students">
		...
		foreach($arResult["PROPERTIES"]["ADMINS"]["VALUE"] as $item) {
            ...
            if($user_id) {
                $bookmark = $dbh->query('SELECT * from a_bookmark WHERE type = 5 AND uz_id = ' . (int) $arResult['USER']['ID'] . ' AND user_id = ' . (int) $user_id)->fetch();
            }
		?>
		<div class="news-item" id="user-<?php echo $arResult['USER']['ID']; ?>">
			...
			<div class="col-9 width-sm content-right" style="padding: 0;">
				<div class="btns right">
					<?if($pageAdmin):?>
					<div class="adm-btns">
						<div class="color-silver js-admins-edit" data-id="<?php echo $arResult['USER']['ID']; ?>" data-vuz-id="<?php echo $arResult['ID']; ?>" data-iblock="2" style="cursor: pointer; border-bottom: 1px dashed #9f9f9f; display: inline-block;">изменить</div>
					</div>
					<?endif?>
				</div>
				<div class="page-info">
