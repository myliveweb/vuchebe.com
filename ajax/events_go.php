...
if($user_id) {
	...
	$test = $dbh->query('SELECT * from a_events_go WHERE id_vuz = ' . $input['id_vuz'] . ' AND key_event = "' . $input['id_event'] . '" AND id_user = ' . $user_id . ' ORDER BY id DESC')->fetch();

	if($test) {
		$count = $dbh->exec('DELETE FROM a_events_go WHERE id_user = ' . $user_id . ' AND id_vuz = ' . $input['id_vuz'] . ' AND key_event = "' . $input['id_event'] . '"');
		$del = 1;
	} else {
		$stmt = $dbh->prepare("INSERT INTO a_events_go (id_user, id_vuz, key_event) VALUES (:id_user, :id_vuz, :key_event)");
	...
	$users = $dbh->query('SELECT * from a_events_go WHERE id_vuz = ' . $input['id_vuz'] . ' AND key_event = "' . $input['id_event'] . '" ORDER BY id DESC')->fetchAll();
}
...
$data = array("status" => "success", 'res' => $result, 'del' => $del);
